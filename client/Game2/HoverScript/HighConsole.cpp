...
using HoverRace::VideoServices::FontSpec;
...
class HighConsole::LogLines
{
	...
	public:
		void Add(const std::string &s, const VideoServices::FontSpec &font, MR_UInt8 color);
		void Clear();
	...
	visible(false), helpVisible(false), cursorOn(true), cursorTick(0)
{
	...
	logFont = new VideoServices::FontSpec("Courier New", 16);
	...
	logLines->Add(PACKAGE_NAME " version " + cfg->GetVersion(),
		VideoServices::FontSpec("Arial", 20, true), 0x0a);
...
void HighConsole::LogLines::Add(const std::string &s, const VideoServices::FontSpec &font, MR_UInt8 color)
{
