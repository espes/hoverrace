...
//-------------------------------------------------------------------//
SDLKeyboard::SDLKeyboard( OIS::InputManager *creator, bool buffered ) :
...
void SDLKeyboard::capture()
{
	...
	for( int i = 0; i < count; ++i )
	{
		...
		if( mBuffered && mListener )
		{
			...
			{
				//FIXME: Fill in text.
				mListener->keyPressed(KeyEvent(this, kc, events[i].key.keysym.unicode));
			}
			else
			{
				mListener->keyReleased(KeyEvent(this, kc, 0));
			}
		}
	}
